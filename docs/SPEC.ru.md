...
- Загрузка файла сборки - бинарный POST (Content-Type
  ...
  проставляется только связью проекта с репозиторием. Клиент их не отправляет.
...
- `GET /projects/{id}/assemblies` - список сборок. Элемент содержит
  ...
  ни курсора (проверено живыми вызовами на двух установках). **Платформа сама удаляет сборки, которыми
  никто не пользуется,** и возраст тут ни при чём - справка вендора называет это
  автоматическим удалением неиспользуемых сборок. Остаются сборка, на которой
  работает приложение, библиотечная сборка, занятая другим проектом, релизная
  сборка, сборка проекта по умолчанию и та, из которой создан репозиторий
  проекта; остальное снимается, когда до него дойдёт очередь. Наблюдалось
  живьём: из семнадцати сборок одной дневной серии в перечне осталась одна - та,
  на которой работает приложение, - а сборка двухмесячной давности жива, потому
  что она у проекта первая. Отсюда требование к клиенту: перечень называть тем,
  что он есть - остатком, а не историей проекта и не её страницей.
- `GET /projects/{id}/assemblies/{version}` - карточка сборки; `DELETE
  ...
  ВЕРСИЯ (`assembly-version`/`project-version`, строка вида `1.0-42`), а не ид
  карточки: на UUID метод отвечает 404 "Assembly with version <uuid> not found".
  Проверено живыми вызовами на двух установках разного возраста - ведут себя
  одинаково; страницы утверждали обратное (только UUID, на версию 400 "Version is
  not a valid UUID"), и следование этому утверждению оставляло карточку
  недостижимой в любой из форм. При этом ид - тоже адрес, который есть на руках:
  его печатает перечень сборок и возвращает загрузка. Поэтому клиент принимает
  обе формы и находит по списку сборок ту версию, которую метод ждёт; значение,
  которого нет ни в одной карточке, называется ненайденным, а не превращается в
  404 из глубин платформы. Отказ по адресу (400 либо 404) заставляет клиента
  попробовать сегментом и ид: ни одна доступная установка этого не требует, но
  описанный страницами 400 откуда-то взялся, а вторая форма стоит одного запроса.
  Учитывать, что версию из манифеста платформа при загрузке перенумеровывает
  по-своему. Удаление сборки платформа отклоняет с 500, пока живо созданное из
  неё приложение (п. 6.9); после того как приложение действительно исчезло, тот
  же запрос проходит. 500 - это ответ, а не непонятый адрес: он поднимается как
  есть и другой формой не переспрашивается.
...
- далее файлы проекта путями `{vendor}/{name}/...` - относительно корня
  ...
  доступные локально библиотеки из `Библиотеки`/`Libraries`; объявленная
  ...
  Разделители путей в архиве - прямые слэши (и на Windows).
...
поэтому дескриптор ищется обоими именами везде - поиск проекта, локальные
библиотеки-зависимости, разбор архива, проверка доступности коммита у стража
схемы.

Версия сборки, если не задана явно: `{базовая версия}-{N+1}`, где N - счётчик
из версии последней сборки проекта той же базовой версии; проект, поднятый на новую
базовую версию, снова начинает с `-1`, какие бы номера ни набрала старая база. Без
последней сборки этой базы суффикс берётся из
...
- где бы они ни лежали, включаются файлы описания клиента SOAP-сервиса:
  ...
  поэтому они отбираются по имени, а не по расширению;
...
- каталог первого уровня - **подсистема**; `Подсистема.yaml`/`Subsystem.yaml`
  **необязателен** (у подсистемы библиотеки его может не быть вовсе), поэтому
  опираться на него при поиске подсистем нельзя;
...
Глобально` (`VisibilityScope: Global`); умолчание - `ВПодсистеме`/`InSubsystem`,
глобальная область пишется явно. Английский дескриптор несёт английские и
ЗНАЧЕНИЯ - значения перечислений читаются в обоих написаниях.
...
`--client-id`, `--client-secret`, `--env-file`, `--timeout` (сек, по умолчанию 60),
`--json`, `--version`.

Флаги подключения – про разговор с платформой, поэтому команды, которые к ней не
обращаются, – `build` и `inspect` – их ОТВЕРГАЮТ, а не глотают молча. Вызов с
`--env-file` читается как сборка, привязанная к стенду, и дважды из-за этого возникал
вопрос, не ходит ли локальная сборка на сервер; вводит в заблуждение именно молчание.
Ничего работавшего это не ломает – флаги ни на что не влияли, – а отказ называет
команды, которые к платформе действительно ходят (`deploy`, `builds upload`). Касается
это только ФЛАГОВ: переменные `ELEMENT_*` и `.env` рядом с проектом есть всегда, и
зависеть от них сборка не должна.
...
`--json` делает это гарантией, а не конвенцией: на время вызова stdout
перенаправляется в stderr, и в настоящий stdout пишется только ответ. Вызывающая
сторона разбирает stdout целиком; без флага любая строка, напечатанная обработчиком
или плагином, остаётся в потоке перед ответом. Отказ и в этом режиме уходит в stderr,
а stdout остаётся пустым.
...
- `apps list [--name --status --include-deleted --brief]`, `apps get [APP_ID]`, `apps find NAME [--include-deleted]`,
  ...
  - `apps ensure` идемпотентно приводит приложение с данным именем в
    ...
    происходит – в том числе `--version-id`. Поэтому у существующего приложения
    ответ несёт ещё `applied` (стоит ли на нём запрошенная сборка) и
    `applied-version-id` (какая стоит): молчание об этом стоило стенда, ушедшего
    в работу на прежней сборке. Привести приложение к сборке в том же прогоне –
    флаг `--apply`; отдельно – `apps apply`. Флаг `--verify` поднимает вердикт о
    приложении, на котором запрошенная сборка уже стоит, со сверки по карточке до
    полной проверки. Созданному приложению `ensure` отвечал `applied: true` на
    веру – оно же создано из этой сборки – а теперь отвечает проверенным
    вердиктом всякий раз, когда проверка была. Существующее приложение никогда не
    пересоздаётся: `delete` +
  ...
  - `apps apply [APP_ID] VERSION_ID` применяет уже загруженную сборку к
    ...
    применилась. До этой команды применение было доступно только инструментом MCP,
    а долгие операции просят запускать CLI фоном.
  ...
  - `--latest-build` - взять источником последнюю сборку проекта (защита от
    пустого каркаса, п. 6.2); `--wait` - дождаться готовности (п. 6.4),
    проверить итог (п. 6.6) и вывести финальную карточку.
  - Дождаться значит проверить. Неудачное применение платформа откатывает на
    ...
    попадает в поле `verify` вывода, а не пройденная проверка – в код возврата 1.
    `--verify` просит проверку отдельно (и сам дожидается готовности: проверять
    ещё создающееся приложение не на чем), `--no-verify` возвращает прежнее
    простое ожидание. Без обоих флагов, как и раньше, ничего не ждётся и ничего
    не проверяется.
- `spaces list`.
...
- `builds list [--project-id --limit --brief]`, `builds get VERSION [--project-id]`,
  ...
  инструмента (`--limit`) или уборка платформы (раздел 4.4: она сама удаляет сборки,
  которыми никто не пользуется, и возраст тут ни при чём). "30 из 30" без этой строки
  читалось как вся история проекта. Про уборку строка судит по ФАКТАМ ОТВЕТА, а не по
  длине перечня: номера сборок одной базовой версии платформа выдаёт подряд, поэтому
  номер, которого в перечне нет, - это уже снятая сборка, и строка называет, скольких
  номеров не хватает.
...
- `deploy [--app-id --project-id --project-dir --output --build-version
  ...
  `
` и `	` ровно там, где его надо читать), `ok` (булево), `dirty`/`dirty-files`
...
- `verify-deploy [APP_ID] [--app-id --version-id --expected-version --since-minutes]` -
  проверка п. 6.1 отдельной командой, ничего не деплоящая: задачи приложения в
  ошибочном статусе за последние `--since-minutes` минут (в их `error-message` файл и
  позиция ошибки компиляции), сверка применённой сборки с ожидаемой (`--version-id` -
  ид загруженной сборки, надёжная сверка; `--expected-version` - строка версии,
  запасная) и контрольный GET на адрес. Отчёт тот же, что у `deploy`, код возврата 0
  только при `ok`. Это то, что нужно скрипту CI после `apps create`: сбой применения
  платформа откатывает молча, и статус `Running` сам по себе ничего не доказывает.
- `probe [--project-dir --output --build-version --name --space-id --keep
  ...
  этого режима в файлах, которых правка не касалась: отказ распознаётся, разбор
  на нём обрывается, `compatibility-refused` называет режим, а
  `messages-dropped` считает производное – чтобы вердикт о СТЕНДЕ не читался как
  вердикт о коде. Код возврата 0 только при `ok`; неудавшаяся
...
Это готовое значение настройки `xbsl.debug.adapterPath` расширения VS Code.
