...
- `GET /projects/{id}/assemblies` - список сборок. Элемент содержит
  ...
  проект платформа ограничивает:** на наблюдавшихся установках их порядка 30
  (видели и 30, и 31), и загрузка новой снимает старую - сборка, на которой
  работает приложение, при этом остаётся; лишняя запись держится до следующей
  загрузки. Наблюдалось живьём: `1.0.2-3` пропала из перечня, как только
  появилась `1.0.2-4`. Отсюда
