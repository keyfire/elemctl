---
title: "Команды"
description: "Справочник команд и параметров elemctl: общие флаги, приложения, сборки, развёртывание и всё остальное."
sidebar:
  label: Команды
  order: 3
---
...
Общие флаги принимаются в любой позиции – и до команды, и после неё: `elemctl --timeout 120 apps list` и `elemctl apps list --timeout 120` равнозначны. Язык вывода переключается флагом `--lang`, переменной `ELEMCTL_LANG` или берётся из локали системы.
...
| `build` | локально собрать архив сборки из исходников (к платформе не обращается: ключи подключения, включая `--env-file`, команда отвергает, номер версии на сервере не занимается) |
| `inspect` | разобрать готовый архив сборки (.xasm/.xlib); к платформе не обращается – ключи подключения, включая `--env-file`, команда отвергает |
...
| `probe` | изолированная проверка компиляции: сборка -&gt; одноразовое приложение -&gt; ошибки с файлом и позицией -&gt; уборка; каталог проекта обязан лежать по схеме {репозиторий}/{Поставщик}/{Имя}/Проект.yaml – по паре Поставщик+Имя платформа опознаёт проект |
| `verify-deploy` | проверить, что сборка действительно применилась к приложению: задачи с ошибками (в них файл и позиция ошибки компиляции), сверка применённой сборки, доступность адреса; сама ничего не разворачивает |
| `branches` | ветки среды разработки |
| `dumps` | дампы приложений |
| `tasks` | задачи приложений (перечень – tasks list [`--app-id` ИД]) |
...
| `--wait` | дождаться готовности приложения (заодно проверяет, что стоит нужная сборка) |
| `--verify` | проверить, что приложение правда работает на запрошенной сборке (подразумевает `--wait`); при неудаче код возврата 1 |
| `--no-verify` | не проверять применённую сборку – только дождаться готовности |
...
| `--allow-data-loss` | разрешить применение, пересоздающее данные объектов (сужение длины, смена типа реквизита, в том числе в табличной части); без флага такое развёртывание отклоняется до сборки. Снятие реквизита или табличной части флага не требует: деплой называет его и идёт дальше |
...
|---|---|
| `-h, --help` | показать эту справку и выйти |
| `--project-dir PROJECT_DIR` | каталог проекта – вида .../{Поставщик}/{Имя} с Проект.yaml внутри (по умолчанию ищется вглубь от текущего) |
| `--output OUTPUT` | каталог для архива (по умолчанию временный) |
...
| `-h, --help` | показать эту справку и выйти |
| `--app-id APP_ID` | то же приложение ключом: deploy и apps ensure принимают только эту форму |
| `--version-id VERSION_ID` | ид загруженной сборки, которую ждём применённой – надёжная сверка (строка версии у нового приложения нумеруется заново) |
| `--expected-version EXPECTED_VERSION` | строка версии вместо ид сборки – запасная сверка |
| `--since-minutes SINCE_MINUTES` | за сколько последних минут считать ошибки задач своими (по умолчанию 30) |
