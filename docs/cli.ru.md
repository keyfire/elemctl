...
| `build` | локально собрать архив сборки из исходников (к платформе не обращается: реквизиты подключения и `--env-file` команде не нужны, номер версии на сервере не занимается) |
