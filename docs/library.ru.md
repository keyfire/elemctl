...
`list_apps()` отдаёт живые приложения: удалённые остаются в перечне платформы
со статусом `Deleted`, и на стенде, живущем не первый месяц, их сотни – за полным
перечнем `include_deleted=True`. `list_apps_counted()` возвращает тот же список в
`items` вместе со счётчиками, о которых сообщают CLI и инструмент MCP, – `total`,
`live` и `shown`.
