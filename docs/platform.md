...
13. **A removal is applied without a question.** A build whose sources no longer have a tabular part is applied as it is: the part goes, and its rows go with it. No refusal and no warning, the task ends well and the application keeps running; that was seen on a catalog with a row in the part. An attribute of a tabular part and an attribute of the object go the same way, with their values. A narrowed length or a changed type recreates the data of the object, so `elemctl deploy` refuses those unless told otherwise. A removal it names before the build and goes on: it is a deliberate edit of the sources, and the line says what the apply takes away.
