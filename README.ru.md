...
Флаг `--json` (в любой позиции) превращает эту конвенцию в гарантию для скриптов:
на время работы команды stdout подменяется на stderr, и в настоящий stdout попадает
только JSON ответа - посторонняя строка туда не пролезет ни из плагина, ни из
библиотеки. Разбирать поток целиком (`json.load`), а не искать в нём первую скобку.
Отказ и с `--json` уходит в stderr, а stdout остаётся пустым: документ в машинном
канале конвейер прочитал бы как ответ.
...
| `get_build` | карточка одной сборки целиком; `version` – ВЕРСИЯ сборки (`1.0-42`), ид тоже принимается и разрешается по перечню |
...
игнорирует), `status` – по статусу целиком. Удалённые приложения скрыты, пока
их не попросят (`include_deleted`): на стенде, живущем не первый месяц, в
перечне сотни карточек, из которых живых единицы. Молча ответ не режется – в
нём счётчики `total` (сколько карточек отдала платформа), `live` (сколько из
них не удалено), `shown` и готовая строка `summary` ("живых 7 из 324"), а сами
карточки в `applications`. Так же ведёт себя `list_projects` (id, имя, вид проекта,
...
иного (0 снимает срез), а сами карточки лежат в `builds`. Рядом с ними
счётчики `total` (сколько сборок отдала платформа), `shown` и строка `summary`:
перечень платформы – не вся история проекта, платформа сама удаляет сборки,
которыми никто не пользуется, и возраст ни при чём (сборка работающего
приложения, первая сборка проекта и релизная остаются), страниц у перечня нет, и
перечень, оставшийся от истории, назван так прямо. Судит строка по фактам ответа:
номера сборок одной базовой версии платформа выдаёт подряд, поэтому номер, которого
в перечне нет, - это сборка, уже снятая уборкой. `get_build` отдаёт одну карточку
целиком и адресуется ВЕРСИЕЙ сборки - ид тоже принимается и находится по перечню,
потому что ид карточки метод адресом не считает.
`get_app`, `delete_app`,
...
Их параметр `verify` дожидается приложения и проверяет, что оно правда работает на
запрошенной сборке: неудачное применение платформа откатывает молча, а созданное
приложение считалось готовым на веру. Отчёт попадает в поле `verify` ответа;
ожидание стоит минут, поэтому такой вызов уместнее фоновым запуском `elemctl`.
...
- Приложение, созданное со статусом `Error`, платформа сопровождает обобщённым
  ...
  `deploy` и `verify` - ходить за причиной в журнал сервера не нужно. Но не всякий
  сбой кончается статусом `Error`: неудачное применение откатывается на прежнюю
  сборку, и приложение поднимается в статусе `Running`, поэтому `--wait` заодно
  проверяет, на какой сборке приложение правда работает, и отвечает кодом
  возврата 1, если не на запрошенной (`--no-verify` возвращает простое ожидание).
