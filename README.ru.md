...
перечень платформы – не вся история проекта, платформа хранит ограниченное число
сборок и при загрузке новой снимает старую (сборка работающего приложения
остаётся), страниц у перечня нет, и упёршийся в предел перечень назван так
прямо.
`get_app`, `delete_app`,
